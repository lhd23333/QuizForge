<p align="center">
  <img src="assets/brand/quizforge-app-icon-1024.png" width="88" alt="QuizForge 图标">
</p>
...
- **Agent 工作区**：从左侧 Agent 打开右侧对话面板，以流式 Markdown 对话搜索、整理、导入和导出；工具状态可展开，生成可真正停止。标准模式对本机命令、题库写入、付费转换和模板操作逐次确认；危险模式只在当前会话明确授权后免确认，切换会话或重启即失效，并始终保留安全校验与日志。
- **复用提示词和模板**：内置两份只读提示词，帮助外部对话式 AI 生成可直接校对导入的 QuizForge Markdown 或候选 TeX 模板；也可维护本地自定义提示词。带图题目可使用安全 ZIP 包或逐题粘贴原图，自定义 TeX 模板必须真实编译验证后才能启用。
...
- [产品全景](docs/PRODUCT.md)：九个产品板块的定位、用户价值、核心原则、安全／兼容边界与当前成熟度。
...
更新时不需要先卸载：正式分发可运行新版安装包并沿用原安装目录；只更新本机时可执行 `.\update_installed.ps1 -DirectBundle`，只重建桌面目录并覆盖程序文件，不生成安装包。题库登记、转换任务、加密密钥、OCR／LLM 配置、Agent Provider／会话／偏好／审计、用户提示词、Skill、模板目录、设备身份和旧兼容文件独立保存在 `%LOCALAPPDATA%\QuizForge` 或用户选择的题库目录，更新器会在覆盖与启动前后核对这些受保护数据。联网更新只在“关于”页由用户主动检查，清单请求只发送版本和平台信息；发现带 SHA-256 和 Authenticode 证书指纹的已签名新版本后，可由用户再次确认并一键下载、覆盖和重启，不会在后台擅自更新。

桌面版使用无边框原生窗口和自绘标题栏，题库、导入、任务、设置等栏目采用固定左侧导航，窄屏自动收为图标轨道；详细操作说明从左下角“帮助”按当前页面打开。桌面窗口内部使用常驻工作区，从左侧切换栏目时资料库只隐藏、不销毁，已打开的 PDF 和分栏状态会在后台保留；浏览器与 Obsidian 模式继续共享同一套业务页面。首期视觉基线采用柔和石墨深色和中性灰工作区，蓝色只承担交互与品牌强调，详见 [CSS 技术约定](docs/CSS_STYLE_GUIDE.md)。
...
- **图片重绘版本历史**：题卡中的图片重绘会保留原图和生成版本，可按缩略图查看模型、时间及附加要求，切换当前版本或删除不再需要的生成版本。
...
- **提示词与带图 Markdown 包**：侧栏“提示词”提供两份只读官方提示词和本地自定义提示词 CRUD，可一键复制或复制为自定义。Markdown 仍可直接粘贴；需要携带原图时，可导入根目录含 `quizforge-import.json`、`questions.md` 和 `assets/` 的 `.qfimport.zip`／`.zip`，也可在校对页逐题粘贴图片。ZIP 会检查路径穿越、符号链接、类型、缺图、重复项和压缩规模，失败时不会留下本轮资产
...
- **Agent、流式执行与权限**：Agent 使用无头像对话、受限 Markdown／KaTeX 和 SSE 增量更新，可展示工具状态并用停止按钮取消当前轮次；断流、取消或不完整工具参数不会执行工具。标准模式逐次审批本机命令、题库写入、付费调用和模板操作。危险模式会先明确提示任意 PowerShell、CMD、Python 命令实际拥有当前 Windows 用户权限；授权只属于当前会话和本次应用运行，免确认不等于绕过参数、ZIP、模板、CSRF、操作锁或审计校验
...
- **软件内资料库**：题库筛选中的“展示题卡”“显示 PDF 文件”和“展示一般 MD”是三个可任意组合的增量开关；勾选后可在文件树中打开对应内容，并复用资料库的预览、编辑、PDF 工具和制卡能力。资料库路由仍用于常驻预览，但不再占用主导航入口。按当前题库目录惰性展开文件树，在同一工作区用常驻标签阅读 UTF-8 Markdown、PDF 和常见图片；桌面版 PDF 使用 WebView2 内置阅读器，切换标签或顶部栏目不会重建。支持左右／上下双分栏、比例拖动和标签跨栏移动。文件和文件夹可拖入其它文件夹，普通拖动为移动、Shift 拖动为复制，也可拖回资料库根目录；同一目录始终按名称排序，不支持拖动自由排序。移动同步已打开标签并保留 Markdown 草稿，Markdown 保存会检查 Obsidian 等外部修改。PDF 可提取、排序、旋转、拆分或合并：输入和输出均为资料库相对路径，合并时每行填写一份资料库 PDF，不提供任意磁盘文件选择器；默认在源文件夹生成“原名-提取／排序／旋转.pdf”“第一份原名-合并.pdf”或“原名-第x-y页.pdf”，已有文件不会被覆盖。DOC／DOCX 只进入索引；DOCX 在用户选择后才转 Markdown 或 PDF，旧 DOC 需先另存为 DOCX。文件接口只允许题库根目录内的可见白名单文件，PDF 与图片只读
- **资料库自然制卡**：Markdown 可选中文字或粘贴内容；PDF 可按全文／指定页面识别，也可在 Windows 桌面版当前阅读画面中分别框选题干与可选解析；PNG／JPG／WEBP／BMP 图片可直接识别制卡并另配解析截图。PDF／图片默认使用 OCR 后的机械拆题，只有显式勾选才调用大模型规范化。单题模式检测到多题会停止，多题模式按边界拆题；智能模式检查题号连续与重复，白名单模式允许跳号。保存位置从已有题集中选择并默认“临时卡片”，可填写单题名称或多题命名前缀；PDF、DOCX 和制卡都在独立的后台资料库任务中执行，失败或重启后标记中断的任务可手动重试
...
- **安全自定义 TeX 模板**：设置页可上传 UTF-8 `.tex`、带清单的 `.tex.zip` 或只读 PDF 参考。可执行模板必须仅含一个 `$body$`、保留 QuizForge 运行时宏，并在禁用 shell escape、受限输入输出和禁止 MiKTeX 自动联网安装的独占目录中，用固定中文、公式、表格、图片与解析样例完成所有声明模式的真实编译；验证绑定源码哈希，改动后自动失效，未选择模板时继续使用内置模板
...
MinerU Token 与 LLM 配置都可在设置页维护；LLM 配置支持新增、编辑、按“导入识别/配图重绘”分别启停，编辑时 API Key 留空会保留原值。新建时可选择 DeepSeek 或阿里云百炼（Qwen）预设，再从常用模型中选择或手填其他模型；页面显示模型上下文参考和推荐最大输出，但不限制自定义模型。勾选“配图重绘”的视觉配置会出现在设置页快速切换区，点模型按钮即可切换，不影响导入识别所用模型；Qwen-VL 等视觉聊天模型输出 TikZ，`gpt-image-*` 自动使用 Images Edit 输出 PNG。
...
自有 MinerU Token、Doc2X Key 与 LLM／Agent Provider API Key 使用 Fernet 对称加密后存储在 `data/.enc_key`（密钥文件）以及 `data/providers.json`、`data/agent_providers.json`、`data/mineru.json`、`data/doc2x_local.json`（加密后的配置）。**这个密钥文件如果被删除或更换，已保存的 Key 会永久无法解密**，只能在设置页重新填写。旧 `data/doc2x.json` 仅作为回退文件保留，新版不读取或改写。上述文件已在 `.gitignore` 中排除，切勿手动提交到 git 或加入公开备份/同步工具。
...
软件版/
  ...
  agent_core.py             Agent 会话持久化、轮次状态、操作锁与取消
  agent_orchestrator.py     Chat Completions/Responses 流与受控工具循环
  agent_provider.py         Agent Provider 加密配置与协议适配
  agent_tools.py / agent_actions.py / agent_approvals.py / agent_upload.py
                             Agent 工具、审批、审计与材料安全暂存
  agent_catalog.py          Skill 与自定义模板目录、迁移和管理接口
  prompt_store.py           官方提示词读取与用户提示词原子存储
  import_bundle.py          带图 Markdown 包校验、资产暂存与回滚
  template_pipeline.py      TeX 模板契约、迁移、哈希和真实预览
  tex_sandbox.py            外部 TeX 资源检查与受限 XeLaTeX 执行
  ...
  assets/                  应用图标、导出资源与受控品牌源文件
    brand/                 QuizForge/WIMath 品牌源文件（见 docs/BRAND_ASSETS.md）
  ...
  frontend/                  讲义／Agent Markdown 源码与 Node 单测（仅开发期）
  static/                    CSS、运行时品牌镜像与已编译前端资源
    brand/                 浏览器/桌面运行时品牌资产镜像
  ...
  data/                       本机私有配置与状态（.gitignore 排除）
    prompts.json              用户自定义提示词
    agent_*.json / agent_audit.jsonl  Agent 配置、会话、目录与审计
    agent_skills/ / agent_templates/  用户 Skill 与自定义模板资源
    banks/<题库摘要>/         各题库的选题、任务、历史与临时状态
